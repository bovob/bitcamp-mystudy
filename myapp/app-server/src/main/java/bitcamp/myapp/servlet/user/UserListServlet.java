package bitcamp.myapp.servlet.user;

import bitcamp.myapp.dao.UserDao;
import bitcamp.myapp.vo.User;

import javax.servlet.*;
import javax.servlet.annotation.WebServlet;
import java.io.IOException;
import java.io.PrintWriter;

// 서블릿 컨테이너에 동적자원을 배치할 때 사용
// 클라이언트 요청이 들어왔을 때 그 요청을 처리할 클래스를 지정한다.
// URL을 이용하여 요청을 처리할 자원을 구분한다.
@WebServlet("/user/list")
public class UserListServlet extends GenericServlet {

    private UserDao userDao;

    @Override
    public void init(ServletConfig config) throws ServletException {
        super.init(); // 기존에 구현된 코드는 그대로 실행하고

        userDao = (UserDao) config.getServletContext().getAttribute("userDao");
    }

    //    @Override
//    public void init(ServletConfig config) throws ServletException {
//        // 서블릿 객체를 생성한 후 바로 호출됨(물론, 생성자가 먼저 호출된다.)
//        // 서블릿이 작업할 사용할 의존 객체를 준비하는 일을 이 메서드에서 수행한다.
//        this.config = config;
//
//        ServletContext ctx = config.getServletContext();
//        userDao = (UserDao) ctx.getAttribute("userDao");
//    }

    @Override
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        // 웹브라우저에서 서블릿을 실행해달라고 요청이 들어오면 이 메서드가 호출된다.
        // 누가 호출하는가? 서블릿 컨테이너가 호출한다.
        res.setContentType("text/html;charset=utf-8");
        // 출력할 컨텐트 타입을 지정하지 않고 출력 스트림을 꺼내면
        // 출력 문자열(UTF-16BE)는 ISO-8859-1 문자집합에 맞춰 인코딩 된다.
        // 만약 UTF-16BE에 있는 문자가 ISO-8859-1에 정의되어 있지 않다면,
        // '?' 문자로 변환된다.

        // " 대신 ' 를 써도됨
        PrintWriter out = res.getWriter();
        out.println("<!DOCTYPE html>");
        out.println("<html>");
        out.println("<head>");
        out.println("    <meta charset='UTF-8'>");
        out.println("    <title>Title</title>");
        out.println("</head>");
        out.println("<body>");

        try {
            out.println("<h1>회원 목록</h1>");
            out.println("<table border='1'>");
            out.println("  <thead>");
            out.println("      <tr><th>번호</th><th>이름</th><th>이메일</th></tr>");
            out.println("  </thead>");
            out.println("  <tbody>");

            for (User user : userDao.list()) {
                out.printf("      <tr><td>%d</td><td><a href='/user/view?no=%1$d'>%s</a></td><td>%s</td></tr>\n",
                        user.getNo(), user.getName(), user.getEmail());
            }

            out.println("  </tbody>");
            out.println("</table>");
        } catch (Exception e) {
            out.println("목록 조회 중 오류 발생!");
        }

        out.println("</body>");
        out.println("</html>");
    }

    @Override
    public void destroy() {
        // 서블릿 컨테이너가 종료되기 전에 해제할 자원이 있다면 이 메서드에서 수행한다.

    }

    @Override
    public String getServletInfo() {
        // 서블릿 컨테이너 관리 화면에서 서블릿의 정보를 출력할 때 이 메서드가 호출된다.
        // 간단히 서블릿에 대한 간단한 정보를 문자열로 리턴하면 된다.
        return "회원 목록 조회";
    }

//    @Override
//    public ServletConfig getServletConfig() {
//        // 내부적으로 서블릿의 정보를 조회할 때 사용할 ServletConfig 객체를 리턴해 준다.
//        // 이 메서드가 리턴할 ServletConfig 객체는
//        // init() 메서드가 호출될 때 파라미터로 넘어온 객체로
//        // 따라서 init() 메서드가 호출될 때 ServletConfig 객체를 보관해 둬야 한다.
//        return this.config;
//    }
}
